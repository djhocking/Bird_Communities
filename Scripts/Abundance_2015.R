...
params<-c("beta.a0", "beta.a1", "beta.a2", "beta.a3", "beta.a4", "beta0", "ln.beta0", "beta1", "beta2", "beta3", "beta4", "meanpavail", "meanpdet", "meanN", "totN", "dens", "N", "navail", "pavail", "pdet", "sigma.0", "beta.p1", "sigma") # "sigma.eps.n", 
...
N_avail <- sim_fit$summary %>%
  as.data.frame(.) %>%
  dplyr::mutate(parameter = rownames(sim_fit$summary)) %>%
  dplyr::filter(., grepl("navail[", parameter, fixed = TRUE))

p_avail <- sim_fit$summary %>%
  as.data.frame(.) %>%
  dplyr::mutate(parameter = rownames(sim_fit$summary)) %>%
  dplyr::filter(., grepl("pavail[", parameter, fixed = TRUE))

p_det <- sim_fit$summary %>%
  as.data.frame(.) %>%
  dplyr::mutate(parameter = rownames(sim_fit$summary)) %>%
  dplyr::filter(., grepl("pdet[", parameter, fixed = TRUE))

N_est <- N_est[ , c("parameter", "50%", "2.5%", "97.5%")]

cbind(N_est, N_min, N_avail[ , 5], p_avail[ , 5], p_det[ , 5])
