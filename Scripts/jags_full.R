...
pi.pa[3, k] <- (1 - pow((1 - p.a[k]), 5)) * pow((1 - p.a[k]), 5)
      ...
      log(lambda[k]) <- exp(ln.beta0) + beta1*Year[k] + beta3*Shrub_stm_total[pyear[k]] + beta4*Tree_stm_total[pyear[k]] + eps.n[point[k]] #  + beta1*VegHgt[k] + beta2*VegHgt[k]*VegHgt[k]
