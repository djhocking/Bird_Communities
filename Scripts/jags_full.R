...
cat("
  model {
    ...
    for(l in 1:n_points) {
      beta0[l] ~ dnorm(mu.point, tau.point) 
      beta.point[l]<-beta0[l]-mu.point
    }
    mu.point~ dnorm(0,0.01)
    tau.point<-pow(sd.point,-2)
    sd.point~dunif(0,5)
    ...
    for(k in 1:nsurveys) { 
      ...
      log(lambda[k]) <- beta0[point[k]] # ln.beta0 + eps.n[point[k]] # + beta1*Year[k] # + beta3*Shrub_stm_total[pyear[k]] + beta4*Tree_stm_total[pyear[k]] #  #  + beta1*VegHgt[k] + beta2*VegHgt[k]*VegHgt[k]
